...
KConfigGroup TheStream::profile()
{
...
bool TheStream::canSeek()
{
    return videoWindow()->m_media->isSeekable();
}

bool TheStream::hasAudio()
{
    return true;
}

bool TheStream::hasVideo()
{
    return videoWindow()->m_media->hasVideo();
}

bool TheStream::hasMedia()
{
...
QSize TheStream::defaultVideoSize()
{
...
QAction *TheStream::aspectRatioAction()
{
    return s_aspectRatioActions[engine()->m_vWidget->aspectRatio()];
}

void TheStream::addRatio(int aspectEnum, QAction *ratioAction)
{
...
QString TheStream::prettyTitle()
{
    ...
    if (hasVideo() && !title.isEmpty())
        ...
        return QUrl::fromPercentEncoding(
            n.left(n.lastIndexOf(QLatin1Char('.'))).replace(QLatin1Char('_'), QLatin1Char(' ')).toUtf8()); // krazy:exclude-qclasses
    } else if (videoWindow()->m_media->currentSource().discType() == Phonon::Cd) {
        return i18n("Track %1/%2",
                    videoWindow()->m_media->metaData().value(QStringLiteral("TRACK-NUMBER")),
...
bool TheStream::hasProfile()
{
    return KSharedConfig::openConfig()->hasGroup(url().toDisplayString());
}

QString TheStream::metaData(Phonon::MetaData key)
{
